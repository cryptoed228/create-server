...
- Use `json:"-"` tag for sensitive fields (password, tokens)
...
### adapter.go — struct and constructor only:
...
### Business methods in separate files by entity:
...
func (a *Adapter) CreateUser(ctx context.Context, user domain.User) (domain.User, error) {
    ...
    err := a.pool.QueryRow(ctx, query, user.Email, user.Name, user.Password).Scan(
...
func (a *Adapter) ListUsers(ctx context.Context, limit, offset int) ([]domain.User, int, error) {
    ...
    var total int
    if err := a.pool.QueryRow(ctx, countQuery).Scan(&total); err != nil {
...
- Adapter methods return `domain.*` types — map DB/external data to domain models inside the adapter
...
6. **Run `make wire`** to regenerate wire_gen.go
7. **Environment** — Add any new env vars to `.env.development`
8. **Migrations** — Add SQL migration in `migrations/` if new tables/columns needed
9. **Swagger** — Add annotations to the handler, run `make swagger`
